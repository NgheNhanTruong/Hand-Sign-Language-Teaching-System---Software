...
model_yolo = torch.hub.load('ultralytics/yolov5', 'custom', path='yolov5/best.pt')
model_yolo.conf = 0.5
...
class capture_video(QThread):
    ...
    def run(self):
        ...
        thresh_score = 85 #minimun score để xem đó là 1 nhận diện tốt 
        sentence_display = ""
        while (self.Object == True) & (self.Action == False):
            ...
            for result in results:
                ...
                num_confidence = round(confidence*100)
                name = result['name']
                clas = result['class']
                x1 = int (result['xmin'])
                y1 = int (result['ymin'])
                ...
                if (name == "B") & (num_confidence > thresh_score) & (cau_hoi_hien_tai == 1)  :
                    ...
                    if (noca == 10):
                        ...
                        noca = 0
                        cau_hoi_ke_tiep = 2                
                if (name == "C") & (num_confidence > thresh_score) & (cau_hoi_hien_tai == 2)  :
                    noca = noca+1
                    if (noca == 10):
                ...
                if (name == "E") & (num_confidence > thresh_score) & (cau_hoi_hien_tai == 4)  :
                    ...
                    if (noca == 10):
                        ...
                        noca = 0
                        cau_hoi_ke_tiep = 5    
                if (name == "D") & (num_confidence > thresh_score) & (cau_hoi_hien_tai == 5)  :
                    noca = noca+1
                    if (noca == 2):
            ...
            with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
                while cap.isOpened():
                    ...
                    if len(sequence) == 30:
                        ...
                        sentence_display = actions[np.argmax(res)]
                        ...
                        sequence = []
                    cv2.putText(image,'{}'.format(str(sentence_display)), (330,25), 
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (16,117,245), 2, cv2.LINE_AA)                        
                    cv2.putText(image, '{}%'.format(score_python), (330,65), 
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (117,245,16), 2, cv2.LINE_AA)    
                    if ret:
